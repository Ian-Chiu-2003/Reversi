...
ST7735_WriteCommand(ST7735_RAMWR);
    ...
    HAL_GPIO_WritePin(ST7735_DC_GPIO_Port, ST7735_DC_Pin, GPIO_PIN_SET); // ? Data ??
    HAL_GPIO_WritePin(ST7735_CS_GPIO_Port, ST7735_CS_Pin, GPIO_PIN_RESET); // ? ?? LCD

    HAL_SPI_Transmit(&ST7735_SPI_INSTANCE, data, 2, HAL_MAX_DELAY);

    HAL_GPIO_WritePin(ST7735_CS_GPIO_Port, ST7735_CS_Pin, GPIO_PIN_SET); // ? ?? LCD
}
...
void ST7735_DrawFilledCircle(uint16_t x0, uint16_t y0, uint16_t r, uint16_t color)
{
    for (int y = -r; y <= r; y++) {
        for (int x = -r; x <= r; x++) {
            if (x * x + y * y <= r * r) {
